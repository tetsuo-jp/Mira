...
suite = testGroup "DFA minimisation"
  [ testProperty "minimised automaton accepts same strings"  prop_minimiseAccept,
    ...
    testProperty "``not'' does not change meaning"  prop_minimiseNot]
...
prop_minimiseNot :: Nfa Int -> Bool
prop_minimiseNot nfa = (minimise . m_not) nfa == (minimise . m_not . minimise) nfa
